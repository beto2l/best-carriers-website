...
for (const page of release.pages) {
  ...
  if ((html.match(/data-service-card/g) || []).length !== 11) failures.push(`${page.entry} must contain 11 static service cards`);
...
for (const route of ["servicios", "services"]) {
  for (const required of ["index.html", "assets/services.css", "assets/services.js", "data/services.json"]) {
